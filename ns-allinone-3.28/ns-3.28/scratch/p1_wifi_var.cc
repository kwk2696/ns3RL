...
void
ClientApp::SetRate (DataRate dataRate) {
	m_dataRate = dataRate;
}
...
ClientApp::ScheduleTx (void) {
	if (m_running) {
		...
		/*else if (m_dataRate != DataRate("0Mbps")) {
			ScheduleTx ();
		}*/
		else {
...
static void SetClientRate (Ptr<Node>);
...
main (int argc, char *argv[])
{
	...
	for (uint32_t i = 0; i < nWifi; i++) {
		...
		if (true) {
			Simulator::Schedule (Seconds(1.1), &SetClientRate, wifiStaNodes.Get (i));
		}
...
static void SetClientRate (Ptr<Node> node) {
	std::vector <DataRate> rate = {DataRate("2Mbps"), DataRate("4Mbps"), DataRate("6Mbps"), DataRate("8Mbps"), DataRate("10Mbps")};
	int rand = std::rand() % 5;
	
	//std::cout << Simulator::Now().GetSeconds() << std::endl;
	std::cout << "rand: " << rand << std::endl;
	Ptr<Application> app = node->GetApplication(0);
	Ptr<ClientApp> clientApp = DynamicCast<ClientApp> (app);
	clientApp -> SetRate(rate[rand]);
	Simulator::Schedule(Seconds(0.1), &SetClientRate, node);
}
