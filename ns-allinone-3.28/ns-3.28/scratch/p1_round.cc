...
int m_segsize = 1024;
std::ofstream rttFile0("Round-RTT0.txt");
std::ofstream rttFile1("Round-RTT1.txt");
std::ofstream rttFile2("Round-RTT2.txt");
...
uint32_t count = 0;
uint32_t RoundRobin (uint32_t info[]) {
	if (count == 0) {
			if(info[0] != 0) {
				count = 1;
				return 0;
