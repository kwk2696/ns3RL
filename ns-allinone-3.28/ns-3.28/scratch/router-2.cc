...
int main (int argc, char** argv)
{
  ...
  p2p.EnableAsciiAll (ascii.CreateFileStream ("router.tr"));
  ...
  Simulator::Schedule (MilliSeconds (samplingPeriod), &GoodputSampling, "router-Goodput.txt", sinkApps,
  ...
  std::cout << "*** Aplication Statistics *** " << std::endl;
  double thr = 0;
  uint32_t totalPacketsThr = DynamicCast<PacketSink> (sinkApps.Get (0))->GetTotalRx();
  thr = totalPacketsThr * 8 / (stopTime * 1000000.0); //Mbps/s
  std::cout << "  Rx Bytes: " << totalPacketsThr << std::endl;
  std::cout << "  Average Goodput: " << thr << " Mbits/s" << std::endl;
